...
describe("[UNIT-CONTACT-CONFIG] Configurações e Helpers Oficiais de Contato", () => {
  it("deve conter o telefone oficial único 5547988030970 e formato (47) 98803-0970", () => {
    expect(CONTACT_CONFIG.phone).toBe("5547988030970");
    expect(CONTACT_CONFIG.displayPhone).toBe("(47) 98803-0970");
    expect(CONTACT_CONFIG.sales.phone).toBe("5547988030970");
    expect(CONTACT_CONFIG.sales.displayPhone).toBe("(47) 98803-0970");
    expect(CONTACT_CONFIG.support.phone).toBe("5547988030970");
    expect(CONTACT_CONFIG.support.displayPhone).toBe("(47) 98803-0970");
  });

  it("getSalesWhatsAppUrl deve retornar wa.me com o número oficial e mensagem padrão", () => {
    const url = getSalesWhatsAppUrl();
    expect(url).toContain("https://wa.me/5547988030970?text=");
  ...
  it("getSalesWhatsAppUrl deve aceitar mensagem customizada codificada", () => {
    ...
    expect(url).toBe(`https://wa.me/5547988030970?text=${encodeURIComponent(custom)}`);
  });
