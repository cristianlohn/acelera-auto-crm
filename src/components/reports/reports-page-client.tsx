...
function ReportsPageContent({ initialData }: { initialData?: ExecutiveReportData } = {}) {
  ...
  // Consumo Dual-Engine consolidado com TanStack Query v5
  const { data: reportData } = useExecutiveReports(
    period,
    isDemoMode,
    isDemoMode ? PERIOD_METRICS[period] : (period === "month" ? initialData : undefined)
  );

  const currentData = reportData || (isDemoMode ? PERIOD_METRICS[period] : (initialData || EMPTY_METRICS));
  const { kpis, funnel, channels, sellers: rawSellers, teamRanking, topVehicles, lostReasons } = currentData;
  const sellersSource = teamRanking || rawSellers;
  const sellers = useMemo(() => sellersSource || [], [sellersSource]);
...
export function ReportsPageClient({ initialData }: { initialData?: ExecutiveReportData } = {}) {
  ...
  if (!existingClient) {
    return (
      <QueryClientProvider client={fallbackQueryClient}>
        <ReportsPageContent initialData={initialData} />
      </QueryClientProvider>
    );
  }

  return <ReportsPageContent initialData={initialData} />;
}

export default ReportsPageClient;
export { ReportsPageClient as ReportsPage };
