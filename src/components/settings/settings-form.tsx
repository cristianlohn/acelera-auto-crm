 ...
 * @description Componente interativo de Configurações, Parâmetros do CRM, Perfil e Gestão de Equipe (SettingsForm).
 *
 * Recebe initialProfile, initialOrganization, initialTeamMembers via props do servidor (0ms de atraso visual).
 */
...
export function SettingsForm({
  ...
  const [profile, setProfile] = useState<UserProfileState>(() => {
    if (initialProfile) {
      return {
        ...
        phone: initialProfile.phone ? formatPhone(initialProfile.phone) : "",
  ...
  const [store, setStore] = useState<StoreState>(() => {
    if (initialOrganization) {
      return {
        ...
        tradeName: initialOrganization.tradeName ?? "",
        cnpj: initialOrganization.cnpj ? formatDocument(initialOrganization.cnpj, "CNPJ") : "",
        phone: initialOrganization.phone ? formatPhone(initialOrganization.phone) : "",
        email: initialOrganization.email ?? "",
        address: initialOrganization.address ?? "",
        businessHours: initialOrganization.businessHours ?? "",
      };
    }
    if (isDemoMode) {
      return {
  ...
  const [crmParams, setCrmParams] = useState<CRMParamsState>(() => {
    if (initialCRMParams) return initialCRMParams;
    return {
  ...
  // Carrega dinamicamente caso não venha hidratado do servidor
  useEffect(() => {
    if (initialProfile && initialOrganization) return;
                ...
                <div className="flex items-center gap-3 border-b pb-4">
                  ...
                  <div>
                    <h2 className="text-sm font-bold text-foreground">
                      {profile.fullName || "Perfil do Usuário"}
                    </h2>
                    <p className="text-xs text-muted-foreground">
                      {profile.email || "email@aceleraauto.com.br"} • {profile.role.toUpperCase()}
                    </p>
                  </div>
                </div>
